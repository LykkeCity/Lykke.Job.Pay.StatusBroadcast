...
using Lykke.Job.Pay.StatusBroadcast.Modules;
...
namespace Lykke.Job.Pay.StatusBroadcast
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            builder.RegisterModule(new JobModule(appSettings.StatusBroadcastJob, log));
        ...
        private static ILog CreateLogWithSlack(IServiceCollection services, AppSettings settings)
        {
            ...
            var dbLogConnectionString = settings.StatusBroadcastJob.Db.LogsConnString;

            // Creating azure storage logger, which logs own messages to concole log
            if (!string.IsNullOrEmpty(dbLogConnectionString) && !(dbLogConnectionString.StartsWith("${") && dbLogConnectionString.EndsWith("}")))
            {
                logToAzureStorage = new LykkeLogToAzureStorage("Lykke.Job.Pay.StatusBroadcast", new AzureTableStorage<LogEntity>(
